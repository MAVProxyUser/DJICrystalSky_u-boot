...
extern void rkclk_set_pll_high(void);
#ifdef CONFIG_ROCKCHIP_POWER_SUPPLY
extern int bq40z50_battery_init(void);
extern int usb_typec_init(void);
extern void power_supply_fsm_init(void);
#endif
...
int board_late_init(void)
{
...
#ifdef CONFIG_ROCKCHIP_POWER_SUPPLY
	bq40z50_battery_init();
	usb_typec_init();
	power_supply_fsm_init();
#endif 
       rkclk_set_pll_high();
	...
	/*Xandy add for send SecureMode to kernel*/
	memset(tmp_buf, 0, 32);
	if(sprintf(tmp_buf, "%d", SecureMode) > 0)
	{
		setenv("secure_mode#", tmp_buf);
	}
