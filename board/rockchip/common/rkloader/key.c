...
__maybe_unused static struct fdt_gpio_state	gBackKey;
__maybe_unused static int rkkey_parse_backkey_dt(const void *blob, struct fdt_gpio_state *key_gpio);
...
#ifdef CONFIG_RECOVERY_KEY_ENABLE	
	if(GetPortState(&key_recovery)) {
		*boot_recovery = 1;
	}
#endif

#ifdef CONFIG_FASTBOOT_KEY_ENABLE
	if(GetPortState(&key_fastboot)) {
		*boot_fastboot = 1;
	}
#endif
	
#ifdef CONFIG_POWERKEY_INTO_LOADER	
	if(GetPortState(&key_power)){
		*boot_rockusb = 1;
	}
#endif
	return 0;
...
#ifdef CONFIG_OF_LIBFDT
	memset(&gBackKey, 0, sizeof(struct fdt_gpio_state));
	rkkey_parse_backkey_dt(gd->fdt_blob, &gBackKey);
#endif

	key_rockusb.type = KEY_INT;
	key_rockusb.key.ioint.name = "rockusb_key";
	key_rockusb.key.ioint.gpio = gBackKey.gpio;
...
static int rkkey_parse_backkey_dt(const void *blob, struct fdt_gpio_state *key_gpio)
{
	int powerkey_node;

	powerkey_node = fdt_path_offset(blob, "/adc/key/back-key");
	if (powerkey_node < 0) {
		printf("no back key node\n");
		return -1;
	}

	fdtdec_decode_gpio(blob, powerkey_node, "gpios", key_gpio);
	key_gpio->flags = !(key_gpio->flags & OF_GPIO_ACTIVE_LOW);

	printf("back key: bank-%d pin-%d\n", RK_GPIO_BANK(key_gpio->gpio), RK_GPIO_PIN(key_gpio->gpio));
	return 0;
}
