...
#define	USB_CRYSTAL_CMD_GET_CHALLENGE	0xF0
#define	USB_CRYSTAL_CMD_PUT_RESPONSE	0xF1

#define	USB_CRYSTAL_CHALLENGE_LEN	16

int rc_auth_in2 = 0;
extern void ep0in_ack(void);
static void	Simple_EncryptAndDecrypt( u8 *pucData, u32 ulLen, u16 usSeq )
{
	#define SIMPLE_KEY "xO$3(-2@#ld*viAQ~ixFE"
	
	static char	*s_pcsimple_key = SIMPLE_KEY;
	static u32	s_ulsimple_key_len = sizeof(SIMPLE_KEY) - 1;
	...
	for (i = 0; i < ulLen; i++)
	{
		...
		if(i & 0x01)
		{
			pucIn[i] = pucIn[i] ^ (u8)(usSeq >> 8);
		}
		else
		{
			pucIn[i] = pucIn[i] ^ (u8)(usSeq & 0xFF);
		}
	}
}

static unsigned char challenge[USB_CRYSTAL_CHALLENGE_LEN];
static void rc_auth_get_challenge(struct usb_device_request *request, struct urb *urb)
{
	int i, status;
	unsigned int seed = (unsigned int)&status;
	
	if (request->wValue != (~request->wIndex & 0xffff))
		return;
		
	usbdbg("%s: len = %d\n", __func__, urb->actual_length);
    srand(seed);
	for (i = 0; i < USB_CRYSTAL_CHALLENGE_LEN; i++)
   		challenge[i] = rand() % 0xff;
	memcpy ((void *)urb->buffer, (void *)challenge, USB_CRYSTAL_CHALLENGE_LEN);
	urb->actual_length = USB_CRYSTAL_CHALLENGE_LEN;
}
/*
return 1 pass,others fail
*/
static int rc_auth_check_response(struct usb_device_request *request, struct urb *urb)
{
	int i;
	if (request->wValue != (~request->wIndex & 0xffff))
		return 0;
	...
	usbdbg("%s: len = %d\n", __func__, urb->actual_length);
	Simple_EncryptAndDecrypt(
			challenge,
			USB_CRYSTAL_CHALLENGE_LEN,
			le16_to_cpu(request->wValue));
	for (i = 0; i < USB_CRYSTAL_CHALLENGE_LEN; i++)
		if (challenge[i] != urb->buffer[i])
			break;
	ep0in_ack();
	return (i >= USB_CRYSTAL_CHALLENGE_LEN);
}

static int rkusb_cdc_setup(struct usb_device_request *request, struct urb *urb)
{
	
	usbdbg("%s: bRequest = %x wValue %x wIndex %x\n", __func__, request->bRequest, request->wValue, (~request->wIndex & 0xffff));
	switch (request->bRequest){
		case USB_CRYSTAL_CMD_GET_CHALLENGE:
			rc_auth_get_challenge(request, urb);
			break;
		case USB_CRYSTAL_CMD_PUT_RESPONSE:
			rc_auth_in2 = rc_auth_check_response(request, urb);
			break;
		default:
			return 1;
	}
...
static void rkusb_init_instances(void)
{
	...
	device_instance->cdc_recv_setup = rkusb_cdc_setup;
...
#ifdef CONFIG_ROCKUSB_TIMEOUT_CHECK
		if (get_timer(TimeOutBase) > (10*1000)) {
			printf("DJI RC Authentic Timeout!\n");
			return 1;//TODO return 1 for test version
		}
		udelay(1000);
#endif
	}
	printf("DJI RC Authentic %lu ms!\n", get_timer(TimeOutBase));
	return 1;
}
