...
static void rk_commandline_setenv(const char *boot_name, rk_boot_img_hdr *hdr, bool charge)
{
	...
	char *sm = getenv("secure_mode#");
	if(sm != NULL)
	{
		if (!strstr(command_line, FASTBOOT_SECUREMODE_BOOTARG)) 
		{
			snprintf(command_line, sizeof(command_line),"%s %s=%s", command_line, FASTBOOT_SECUREMODE_BOOTARG, sm);
			printf("%s\n",command_line);
		}
	}
