...
#define COMPAT_ROCKCHIP_FAN "pwm-fan"
...
struct pwm_fan {
	...
	int node;			/*device node*/
	int status;
	struct fdt_gpio_state fan_en;
	unsigned int	period; 	/* in nanoseconds */
	int *levels;
};
struct pwm_fan fan;

static int rk_fan_parse_dt(const void *blob) 
{
	u32 data[3];
	int pwm_node;
	fan.node = fdt_node_offset_by_compatible(blob,
					0, COMPAT_ROCKCHIP_FAN);
	if (fan.node < 0) {
		debug("can't find dts node for fan\n");
		fan.status = 0;
		return -ENODEV;
	}
	if (!fdt_device_is_available(blob,fan.node)) {
		debug("device fan is disabled\n");
		fan.status = 0;
		return -EPERM;
	}
	fdtdec_decode_gpio(blob, fan.node, "enable-gpio", &fan.fan_en);
	fan.fan_en.flags = !(fan.fan_en.flags  & OF_GPIO_ACTIVE_LOW);
	if (fdtdec_get_int_array(blob, fan.node, "pwms", data,
			ARRAY_SIZE(data))) {
		debug("Cannot decode PWM property pwms\n");
		fan.status = 0;
		return -ENODEV;
...
int rk_fan_pwm_config(void)
{
	if (!fan.node)
	{
		rk_fan_parse_dt(gd->fdt_blob);
		gpio_direction_output(fan.fan_en.gpio, 0);
	}

	rk_iomux_config(RK_PWM0_IOMUX+fan.id);
	return 0;
}

void rk_fan_ctrl(void)
{
	rk_fan_pwm_config();
}
...
struct motor {
	int node;			/*device node*/
	int status;
	struct fdt_gpio_state motor_en;
};

struct motor motor;

static int rk_motor_parse_dt(const void *blob) 
{
	motor.node = fdt_node_offset_by_compatible(blob,
					0, "rockchip,motor-ctrl");
	if (motor.node < 0) {
		debug("can't find dts node for motor\n");
		motor.status = 0;
		return -ENODEV;
	}
	if (!fdt_device_is_available(blob,motor.node)) {
		debug("device motor is disabled\n");
		motor.status = 0;
		return -EPERM;
	}
	fdtdec_decode_gpio(blob, motor.node, "enable-gpio", &motor.motor_en);
	motor.motor_en.flags = !(motor.motor_en.flags  & OF_GPIO_ACTIVE_LOW);

	return 0;
}
int rk_motor_config(int enable)
{
	int ret;
	if (!motor.node) {
#ifdef CONFIG_OF_LIBFDT
		debug("rk motor parse dt start.\n");
		ret = rk_motor_parse_dt(gd->fdt_blob);
		debug("rk motor parse dt end.\n");
		if (ret < 0)
			return ret;
#endif
		gpio_direction_output(motor.motor_en.gpio, motor.motor_en.flags);
	}

	if (enable)
		gpio_set_value(motor.motor_en.gpio, 1);
	else
		gpio_set_value(motor.motor_en.gpio, 0);
	return 0;
}

void rk_motor_ctrl(void)
{
	rk_motor_config(1);
	mdelay(150);
	rk_motor_config(0);
