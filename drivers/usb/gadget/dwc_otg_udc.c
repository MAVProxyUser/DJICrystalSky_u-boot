...
static void dwc_otg_setup(struct usb_endpoint_instance *endpoint)
{
	...
	if ((ep0_urb->device_request.bmRequestType & USB_REQ_DIRECTION_MASK) == USB_REQ_HOST2DEVICE 
		&& (ep0_urb->device_request.bmRequestType & USB_REQ_TYPE_MASK) != USB_STANDARD_REQUEST
		&& (le16_to_cpu(ep0_urb->device_request.wLength)) > 0)
	{
		ReadEndpoint0(Ep0PktSize, (void *)Ep0Buf);
		printf("non standard request copy data to buffer\n");//warning:no delete it
		memcpy((void *)ep0_urb->buffer, (void *)Ep0Buf, le16_to_cpu(ep0_urb->device_request.wLength));
	}
