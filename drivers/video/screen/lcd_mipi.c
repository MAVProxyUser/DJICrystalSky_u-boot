...
{   
	...
	if (screen->lcd_en_gpio != INVALID_GPIO) {
		printk("===============> lcd_en_gpio: %d \n",!screen->lcd_en_atv_val);
		gpio_direction_output(screen->lcd_en_gpio, !screen->lcd_en_atv_val);
		msleep(5);
		gpio_direction_output(screen->lcd_en_gpio, screen->lcd_en_atv_val);
		msleep(500);
	...
	if (screen->lcd_rst_gpio != INVALID_GPIO) {
		gpio_direction_output(screen->lcd_rst_gpio, screen->lcd_rst_atv_val);
		printk("===============> lcd_rst_gpio: %d \n",screen->lcd_rst_atv_val);
		msleep(10);
		gpio_direction_output(screen->lcd_rst_gpio, !screen->lcd_rst_atv_val);
		printk("===============> lcd_rst_gpio: %d \n",!screen->lcd_rst_atv_val);
		msleep(screen->lcd_rst_delay);
...
{
	if (screen->lcd_en_gpio != INVALID_GPIO) {
		printk("%s lcd_en_gpio: %d \n",__func__, !screen->lcd_en_atv_val);
		gpio_direction_output(screen->lcd_en_gpio, !screen->lcd_en_atv_val);
		msleep(5);
		gpio_direction_output(screen->lcd_en_gpio, screen->lcd_en_atv_val);
		printk("%s lcd_en_gpio: %d \n",__func__, screen->lcd_en_atv_val);
	...
	if (screen->lcd_rst_gpio != INVALID_GPIO) {
		gpio_direction_output(screen->lcd_rst_gpio, screen->lcd_rst_atv_val);
		printk("%s lcd_rst_gpio: %d \n",__func__, screen->lcd_rst_atv_val);
		msleep(screen->lcd_rst_delay);
		gpio_direction_output(screen->lcd_rst_gpio, !screen->lcd_rst_atv_val);
		printk("%s lcd_rst_gpio: %d \n",__func__, !screen->lcd_rst_atv_val);
		msleep(screen->lcd_rst_delay);
