...
#ifndef CONFIG_RK_UART_ZS600

//Xandy add for dji uart get string
void dji_uart_getstr(char *string)
{
	int i=0;

	while(i < sizeof(string))
	{
		if(rk_uart_tstc(DJI_UART_NUM))
			string[i++] = rk_uart_recvbyte(DJI_UART_NUM);

		//printf("%2x ", rk_uart_recvbyte(DJI_UART_NUM));
	}
}

void dji_uart_putstr(char *string)
{
	while(*string)
		rk_uart_sendbyte(DJI_UART_NUM, *string++);
}
...
#ifndef CONFIG_RK_UART_ZS600
	rk_uart_init(DJI_UART_NUM, CONFIG_BAUDRATE);
#endif
	return ret;
}
